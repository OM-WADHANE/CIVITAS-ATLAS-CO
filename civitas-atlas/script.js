...
/* ═══════════════════════════════════════════
   8. CONTACT FORM
   Validates → opens mail client in a hidden
   iframe trick so the page never navigates
   away → immediately shows the success state.
════════════════════════════════════════════ */
const CONTACT_EMAIL = 'contact.civitasatlas.in@gmail.com';
...
function initContactForm() {
  ...
  form.addEventListener('submit', (e) => {
    ...
    const senderEmail  = (qs('#email',        form)?.value || '').trim();
    const organization = (qs('#organization', form)?.value || '').trim();
    const message      = (qs('#message',      form)?.value || '').trim();

    // ── Build mailto: URL ────────────────────
    const subject = encodeURIComponent(
      `[Civitas Atlas] Inquiry from ${name}`
    );
    const body = encodeURIComponent(
      ...
      `Message:\n${message}\n\n` +
      `---\nSent via civitasatlas.in contact form`
    );
    const mailtoURL = `mailto:${CONTACT_EMAIL}?subject=${subject}&body=${body}`;

    // ── Trigger mail client via hidden iframe ─
    // This fires the mailto: handler without navigating the page away.
    let iframe = document.getElementById('mailtoFrame');
    if (!iframe) {
      iframe = document.createElement('iframe');
      iframe.id = 'mailtoFrame';
      iframe.style.cssText = 'display:none;width:0;height:0;border:0;position:absolute;';
      document.body.appendChild(iframe);
    }
    iframe.src = mailtoURL;

    // ── Swap form → success card immediately ─
    form.setAttribute('hidden', '');       // hide the form
    success.removeAttribute('hidden');     // reveal the success card
  });
}
